...
```java tab="Code"
public static void lebenAnzeigen(String name, int leben, int maxLeben) {
    double prozent = (leben * 100.0) / maxLeben;
    System.out.println(name + ": " + leben + "/" + maxLeben + 
                      " (" + prozent + "%)");
}

public static void main(String[] args) {
    lebenAnzeigen("Spieler", 75, 100);
    lebenAnzeigen("Boss", 450, 500);
}
```
...
```java tab="Code"
public static int schadenBerechnen(int basisSchaden, int waffenSchaden, boolean istCritical) {
    int gesamtSchaden = basisSchaden + waffenSchaden;
...
public static void main(String[] args) {
    int schaden1 = schadenBerechnen(10, 15, false);  // 25
    int schaden2 = schadenBerechnen(10, 15, true);   // 50
    
    System.out.println("Normaler hit: " + schaden1);
    System.out.println("Critical hit: " + schaden2);
}
```
...
public static void nachrichtAnzeigen(String nachricht, int wiederholungen) {
...
public static void main(String[] args) {
    nachrichtAnzeigen("Hallo");           // Erste Version
    nachrichtAnzeigen("Hallo", 3);        // Zweite Version
}
```

<Callout type="info" title="Methodenüberladungsregeln">
  Methoden werden als anders angesehen, wenn sie eines der folgende haben:
...
2. **Kreisrechner**: Erstelle Methoden zum Kalkulieren von:
   * Fläche: `π × radius²`
   * Umfang: `2 × π × radius`
   * Nutze `Math.PI` für π
...
4. **Passwort-Validator**: Erstelle eine Methode, die überprüft, ob ein Passwort gültig ist:
...
// Falsch - Falscher return type
public static int getText() {
    return "Hallo";  // Fehler! Sollte int returnen, nicht String
}
