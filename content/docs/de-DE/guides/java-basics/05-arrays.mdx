...
zahlen = new int[5];    // Array mit 5 Feldern erstellen
```
...
<Callout type="info" title="Array Größe">
  ...
  ```java
  int[] inventar = new int[9]; // Immer genau 9 Slots
  ```
</Callout>
...
```java
String[] spieler = {"Alice", "Bob", "Charlie"};

System.out.println(spieler[0]); // "Alice"
System.out.println(spieler[1]); // "Bob"
System.out.println(spieler[2]); // "Charlie"
```
...
leben[1] = 60;  // Ändert Bobs Leben auf 60

System.out.println(leben[0]); // 100
System.out.println(leben[1]); // 60
System.out.println(leben[2]); // 90
```
...
System.out.println(raster[0][0]); // 1
System.out.println(raster[1][2]); // 6
System.out.println(raster[2][1]); // 8
```
...
for (int row = 0; row < raster.length; row++) {
    for (int col = 0; col < raster[row].length; col++) {
        System.out.print(raster[row][col] + " ");
    }
    System.out.println();
}
```
