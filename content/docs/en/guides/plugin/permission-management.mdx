...
- Check if a user has a permission (it checks both user and group permisions for the given player UUID)
...
Set<String> permissions = new HashSet<>();
PermissionsModule perms = PermissionsModule.get();
perms.removeGroupPermission(groupName, perms);
```
...
```java
PermissionsModule perms = PermissionsModule.get();
perms.removeUserFromGrgetGroupsForUseroup(playerUUID);
```
